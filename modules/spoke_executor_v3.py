...
class SpokeExecutor2:
    ...
    async def _prune_node_list(self, nlq: str, node_list: List[Dict], next_step_desc: str) -> List[Dict]:
        ...
        if len(node_list) > 50:
            current_threshold = int(self.pruning_threshold * 2) # Tăng 100% nếu input quá nhiều
    ...
    async def _execute_logic_step(self, step: Dict[str, Any], store: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Handles UNION and INTERSECTION on lightweight nodes for N inputs.
        Refactored to handle 2 or more lists dynamically.
        """
        logic_op = step["logic"]
        input_names = step.get("inputs", [])
        
        if not input_names or len(input_names) < 2:
            print(f"    ! Warning: Logic step '{logic_op}' requires at least 2 inputs. Inputs provided: {input_names}")
            # Fallback: return the first list if available, else empty
            return store.get(input_names[0], []) if input_names else []

        print(f"    -> Logic: {logic_op} on {len(input_names)} sets: {input_names}")

        # 1. Collect all node dictionaries (Nodes mapped by Identifier)
        # Structure: [ {id1: node1, id2: node2}, {id3: node3...}, ... ]
        list_of_node_maps = []
        for name in input_names:
            node_list = store.get(name, [])
            node_map = await self._get_nodes_by_identifier(node_list)
            list_of_node_maps.append(node_map)

        if not list_of_node_maps:
            return []

        # 2. Perform Set Operations
        if logic_op == "UNION":
            # Start with the first map and update with others
            # Result: Dictionary containing ALL unique nodes from ALL lists
            union_map = list_of_node_maps[0].copy()
            for other_map in list_of_node_maps[1:]:
                union_map.update(other_map)
            
            print(f"    -> Result: UNION size = {len(union_map)}")
            return list(union_map.values())
        
        elif logic_op == "INTERSECTION":
            # Get IDs (keys) from all maps
            # sets_of_ids = [ {'id1', 'id2'}, {'id2', 'id3'}, ... ]
            sets_of_ids = [set(m.keys()) for m in list_of_node_maps]
            
            # Find common IDs across ALL sets using set.intersection unpacking
            common_ids = set.intersection(*sets_of_ids)
            
            # Reconstruct the result list using one of the maps (e.g., the first one)
            # Since IDs are common, the node data exists in all maps (logic wise)
            reference_map = list_of_node_maps[0]
            result_nodes = []
            for uid in common_ids:
                # We prioritize the node data from the first set, 
                # but could logically pick from any since ID matches.
                # Just ensure we get a valid object.
                node_obj = reference_map.get(uid)
                
                # Fallback: check other maps if for some reason missing (edge case)
                if not node_obj: 
                    for m in list_of_node_maps:
                        if uid in m:
                            node_obj = m[uid]
                            break
            ...
            print(f"    -> Result: INTERSECTION size = {len(result_nodes)}")
            return result_nodes
